...
function readArticleDetail(url,callback){
    ...
    request(url,function(err,res){
        ...
        //获取文章内容
        var content =$('.articalContent').text();

        //从url中获取id
        //http://blog.sina.com.cn/s/blog_4be2e2d20102vtri.html
        var s = url.match(/blog_([a-zA-Z0-9]+)\.html/);

        if (Array.isArray(s)) {
            var id = s[1];
        }
        callback(null,{tags:tags,content:content,id:id,url:url});
    });
}

exports.readArticleList=readArticleList;
exports.readArticleDetail=readArticleDetail;
